...
def collate_fn(batch, en_vocab, de_vocab):
    ...
    src_batch = [torch.cat([seq, torch.full((max_len_src - len(seq),), en_vocab["<pad>"], dtype=torch.long)]) for seq in src_batch]
    tgt_batch = [torch.cat([seq, torch.full((max_len_tgt - len(seq),), de_vocab["<pad>"], dtype=torch.long)]) for seq in tgt_batch]
